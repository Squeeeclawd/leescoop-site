...
Parent must test the supported replacement routine/review models, then refresh each route's exact `model` (`provider/model`), `auth:"oauth"`, `verifiedAt`, `expiresAt`, secret-free `evidence`, and actual `successfulInferenceReceipt`. Set `routes.review.sessionKey` to the actual supported reviewer session. Never relabel old receipts or extend timestamps without new proof. The helper checks fresh route evidence and exact export session, actual response provider/model/response ID, successful tool result, report hash and input hash. Candidate `workflowEvidence.routine` and `.review` must name those configured models with real fresh `completedAt` and `receipt` values. Health checks report model agreement only, not inference authenticity or publication.
