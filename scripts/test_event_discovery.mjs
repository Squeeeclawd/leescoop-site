...
const handlers = {};
let refresh;
const document = {hidden:false, querySelectorAll(s) { return ({'.home-tile':tiles,'[data-filter-date-window]':dates,'[data-filter-kind]':kinds})[s] ?? []; }, querySelector() {return null;}, getElementById(s) {return ids[s];}, addEventListener(k,f) {handlers[k]=f;} };
vm.runInNewContext(script, {document,window:{setInterval(f){refresh=f;}},Date:Clock,Intl,HTMLElement:Element,HTMLInputElement:Element,HTMLFormElement:Element,HTMLButtonElement:Element});
const click = e => handlers.click({target:e,preventDefault(){}});
const shown = () => tiles.flatMap((t,i)=>t.hidden?[]:[i]);
assert.deepEqual(shown(),[0,1,2,3,4,6]);
...
click(ids['filter-clear-all']); assert.deepEqual(shown(),[0,2,3,4,6]);
